...
class Transformer(Equipment):
    ...
    def __init__(self, S, Uhigh, Ulow, Usc=0.105, Inl=0.1, Psc=0.1, Pnl=0.1, scheme_high='Y', scheme_low='D', group=11):
        ...
        self.Ihigh = _Var(val=self.S / (3**0.5 * self.Uhigh), name='Iном.вн', desc='номинальный ток стороны ВН', unit='А', n_digits=0)
        self.Ilow = _Var(val=self.S / (3**0.5 * self.Ulow), name='Iном.нн', desc='номинальный ток стороны НН', unit='А', n_digits=0)
    ...
    def __calc_impedances(self):
        self.Zsc = _Var(val=self.Usc * self.Uhigh**2/self.S, name='Zкз', desc='сопротивление трансформатора в режиме КЗ', unit='Ом', n_digits=2)
        self.Rsc = _Var(val=self.Psc/ 3 / self.Ihigh**2, name='Rкз', desc='активное сопротивление трансформатора в режиме КЗ', unit='Ом', n_digits=2)
        self.Xsc = _Var(val=(self.Zsc**2 - self.Rsc**2)**0.5, name='Xкз', desc='реактивное сопротивление трансформатора в режиме КЗ', unit='Ом', n_digits=2)
        self.Znl = _Var(val=self.Uhigh**2/self.S / self.Inl, name='Zхх', desc='сопротивление трансформатора в режиме ХХ', unit='Ом', n_digits=0)
        self.Rnl = _Var(val=self.Uhigh**2/self.Pnl, name='Rхх', desc='активное сопротивление трансформатора в режиме ХХ', unit='Ом', n_digits=0)
        def get_x(z, r):
        ...
        self.Xnl = _Var(val=get_x(self.Znl.val, self.Rnl.val), name='Xхх', desc='реактивное сопротивление трансформатора в режиме ХХ', unit='Ом', n_digits=0)
...
class System(Equipment):
    ...
    """
    def __init__(self, Unom, Isc3, tau=0.03):
        ...
        self.Isc2 = _Var(val=Isc3 * 3**0.5/2, name='Iкз(2ф)', desc='ток двухфазного КЗ на шинах системы', unit='А', n_digits=0)
        self.tau = _Var(val=tau, name='tau', desc='постоянная времени сети', unit='с', n_digits=2)
        self.__calc_impedances()

    def __calc_impedances(self):
        self.Z = _Var(val=self.Unom / 3**0.5 / self.Isc3, name='Z', desc='сопротивление системы', unit='Ом', n_digits=2)
        self.R = _Var(val=self.Z / (1 + (2*np.pi*50*self.tau.val)**2)**0.5, name='R', desc='активное сопротивление системы', unit='Ом', n_digits=2)
        self.X = _Var(val=(self.Z**2 - self.R**2)**0.5, name='X', desc='реактивное сопротивление системы', unit='Ом', n_digits=2)
...
class Load(Equipment):
    ...
    def __calc_impedances(self):
        self.Z = _Var(val=self.Unom**2 / self.Snom, name='Zнагр', desc='сопротивление нагрузки', unit='Ом', n_digits=2)
        self.R = _Var(val=self.Z * self.cos_nom, name='Rнагр', desc='активное сопротивление нагрузки', unit='Ом', n_digits=2)
        if self.__type == 'L':
...
class CT(Equipment):
    ...
    def __init__(self, I1nom, I2nom, Knom=20, Snom=50, cos_nom=0.8,  r2=10., x2=0., Sload=6.3, cos_load=1.0):
        ...
        self.I1faultnom = _Var(val=self.I1nom*self.Knom, name='I1ном кз', desc='номинальный предельный допустимый ток КЗ', unit='А', n_digits=0)
        ...
        self.cos_load = _Var(val=cos_load, name='cos нагр', desc='фактический коэффициент мощности', unit='', n_digits=2)
    ...
    def __calc_impedances(self):
        Rnom, Xnom = self.Snom.val * self.cos_nom.val / self.I2nom.val**2, self.Snom.val * np.sqrt(1 - self.cos_nom.val**2) / self.I2nom.val**2
        Rload, Xload = self.Sload.val * self.cos_load.val / self.I2nom.val**2, self.Sload.val * np.sqrt(1 - self.cos_load.val**2) / self.I2nom.val**2
        self.rnom = _Var(val=Rnom, name='Rном', desc='номинальное активное сопротивление вторичной нагрузки', unit='Ом', n_digits=3)
        self.xnom = _Var(val=Xnom, name='Xном', desc='номинальное реактивное сопротивление вторичной нагрузки', unit='Ом', n_digits=3)
        self.rload = _Var(val=Rload, name='Rфакт', desc='фактическое активное сопротивление вторичной нагрузки', unit='Ом', n_digits=3)
        self.xload = _Var(val=Xload, name='Xфакт', desc='фактическое реактивное сопротивление вторичной нагрузки', unit='Ом', n_digits=3)
    ...
    def calc_saturation(self, Isc, tau, Kr=0.86):
        ...
        # Параметр режима
        A = self.I1nom.val * self.Knom.val * ((R2+Rnom)**2+(X2+Xnom)**2)**0.5 / (Isc * ((R2+Rload)**2 + (X2 + Xload)**2)**0.5)
        # угол нагрузки
        ...
        denom = np.sin(alfa) * np.exp(-t / tau) + np.cos(alfa) * w * tau * (1 - np.exp(-t / tau)) - np.sin(w * t + alfa) + 0.000001
